        ...
        <td>
          <small>${nombreEnf}</small>
              <div>
                ...
                ${ isVeterinario() ? '<button class="btn-editar" title="Editar">✏️</button><button class="btn-eliminar" title="Eliminar">🗑️</button>' : '' }
              </div>
        </td>
      `;
    } else {
      ...
      if(t.idTarjeta){
        ...
        contenidoVisualizar.innerHTML = `
          <div class="detalle item">
            <h4>Animal</h4>
            <strong>Nombre:</strong><p>${anim.nombreAnimal || anim.nombre || ''}</p>
            <strong>Arete:</strong><p>${anim.numArete || ''}</p>
            <h4>Enfermedad</h4>
            <strong>Nombre:</strong><p>${enf.nombreEnfermedad || ''}</p>
            <strong>Tipo:</strong><p>${enf.tipoEnfermedad || ''}</p>
            <h4>Tratamiento</h4>
            <strong>Nombre:</strong><p>${trat.nombreTratamiento || ''}</p>
          </div>
        `;
      } else {
        contenidoVisualizar.innerHTML= `
